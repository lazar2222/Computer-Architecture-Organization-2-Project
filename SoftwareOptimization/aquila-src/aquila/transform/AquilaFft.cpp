...
namespace Aquila
{
    ...
    void AquilaFft::ifft(SpectrumType spectrum, double x[])
    {
        ...
        for (k = 0; k < N-3; k+=4)
        {
            abss[0]=std::abs(spectrumCopy[k]);
            abss[1]=std::abs(spectrumCopy[k+1]);
            abss[2]=std::abs(spectrumCopy[k+2]);
            abss[3]=std::abs(spectrumCopy[k+3]);
            __m256d xs=_mm256_loadu_pd(abss);
            xs=_mm256_div_pd (xs,NS);
            _mm256_storeu_pd(x+k,xs);
        }
        for (; k < N; k++)
        {
