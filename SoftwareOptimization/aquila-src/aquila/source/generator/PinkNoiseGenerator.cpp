...
namespace Aquila
{
    ...
    void PinkNoiseGenerator::generate(std::size_t samplesCount)
    {
        ...
        __m256d amp=_mm256_set_pd (m_amplitude,m_amplitude,m_amplitude,m_amplitude);

        std::size_t i;
        for (i = 0; i < samplesCount-3; i+=4)
        {
            __m256d samp=_mm256_set_pd (pinkSample(),pinkSample(),pinkSample(),pinkSample());
            samp=_mm256_mul_pd(samp,amp);
            _mm256_storeu_pd(m_data.data(),samp);
        }

        for (; i < samplesCount-3; i++)
        {
