...
namespace Aquila
{
    ...
    double MelFilter::apply(const SpectrumType& dataSpectrum) const
    {
        ...
        for (i=0; i < N-3; i+=4)
        {
            abss[0]=std::abs(dataSpectrum[i]);
            abss[1]=std::abs(dataSpectrum[i+1]);
            abss[2]=std::abs(dataSpectrum[i+2]);
            abss[2]=std::abs(dataSpectrum[i+3]);
            __m256d xs=_mm256_loadu_pd(abss);
            __m256d ys=_mm256_loadu_pd(m_spectrum.data()+i);
            xs=_mm256_mul_pd (xs,ys);
            _mm256_storeu_pd(abss,xs);
            value += abss[0]+abss[1]+abss[2]+abss[3];
        }
        for (;i < N; i++)
        {
