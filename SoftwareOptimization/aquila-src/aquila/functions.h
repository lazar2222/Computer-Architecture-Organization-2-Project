...
namespace Aquila
{
                                                  ...
                                                  const std::vector<double>& v2)
    {
        ...
        std::size_t i = 0,size = v1.size();
        double arr[4];
        for (; i < size-3; i+=4)
        {
            __m256d v1s=_mm256_loadu_pd(v1.data()+i);
            __m256d v2s=_mm256_loadu_pd(v2.data()+i);
            v1s=_mm256_sub_pd(v1s,v2s);
            v1s=_mm256_mul_pd(v1s,v1s);
            _mm256_storeu_pd(arr,v1s);
